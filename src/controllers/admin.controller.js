...
const adminPetControllerAgregar = async (req, res) => {
  try {
    const { name, species, breed, adoption_status, entry_date } = req.body;
    ...
    const mascota = await Pet.create({
      ...
      adoption_status,
      entry_date,
      photo_url: req.file.filename ?? 'default.jpg'
    });
...
const adminPetControllerEditSave = async (req, res) => {
  try {
    const { name, species, breed, adoption_status, entry_date, photo_url } =
      req.body;
    const mascota = await Pet.findByPk(req.params.id);
    if (!mascota) {
      return res.status(404).render('404', { error: 'Mascota no encontrada' });
...
const adminSolicitudAprobar = async (req, res) => {
  try {
    ...
    if (!solicitud) {
      ...
      return res
        .status(404)
        .render('404', { error: 'Solicitud no encontrada' });
    }
